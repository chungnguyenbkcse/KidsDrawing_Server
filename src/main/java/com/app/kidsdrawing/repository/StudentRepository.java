...
public interface StudentRepository extends JpaRepository <Student, Long>{
    ...
    @Query("SELECT DISTINCT e FROM Student e JOIN FETCH e.user u JOIN FETCH e.parent p JOIN FETCH e.userRegisterJoinSemesters urj JOIN FETCH urj.semesterClass sc JOIN FETCH sc.semester  s JOIN FETCH s.holidays JOIN FETCH sc.course c JOIN FETCH sc.schedules sch JOIN FETCH sch.lessonTime  WHERE p.id = ?1 AND (u.deleted = FALSE OR u.deleted IS NULL)  ORDER BY e.id")
    List<Student> findByParentId1(Long id);

    @Query("SELECT DISTINCT x FROM Student x JOIN FETCH x.user u  JOIN FETCH x.parent p JOIN FETCH x.userRegisterJoinContests e JOIN FETCH e.student JOIN FETCH e.contest c JOIN FETCH c.userRegisterJoinContests JOIN FETCH c.contestSubmissions cs JOIN FETCH cs.userGradeContestSubmissions JOIN FETCH c.artAges JOIN FETCH c.artTypes WHERE u.id = ?1 AND u.deleted != TRUE ORDER BY x.id")
    List<Student> findByParentId2(Long id);

    @Query("SELECT DISTINCT e FROM Student e JOIN FETCH e.user u JOIN FETCH e.parent p JOIN FETCH e.userRegisterJoinSemesters urj JOIN FETCH urj.semesterClass sc JOIN FETCH sc.semester  s JOIN FETCH sc.course c JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes WHERE p.id = ?1 AND (u.deleted = FALSE OR u.deleted IS NULL)  ORDER BY e.id")
    ...
    @Query("SELECT DISTINCT e FROM Student e JOIN FETCH e.user u JOIN FETCH e.parent p JOIN FETCH e.userRegisterJoinSemesters urj JOIN FETCH urj.semesterClass sc JOIN FETCH sc.semester  s JOIN FETCH s.holidays JOIN FETCH sc.course c JOIN FETCH sc.schedules sch JOIN FETCH sch.lessonTime  WHERE p.id = ?1 AND urj.status = 'Completed' AND (u.deleted = FALSE OR u.deleted IS NULL)  ORDER BY e.id")
    ...
    @Query("SELECT DISTINCT e FROM Student e JOIN FETCH e.user u JOIN FETCH e.userRegisterJoinSemesters WHERE e.id = ?1 AND (u.deleted = FALSE OR u.deleted IS NULL) ")
