...
public interface SemesterClassRepository extends JpaRepository <SemesterClass, Long>{
    ...
    @Query("SELECT  DISTINCT e FROM SemesterClass e JOIN FETCH e.semester  JOIN FETCH e.course c JOIN FETCH e.schedules sch JOIN FETCH sch.lessonTime JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes  ORDER BY e.id")
    ...
    @Query("SELECT DISTINCT e FROM SemesterClass e JOIN FETCH e.semester  JOIN FETCH e.course c JOIN FETCH e.userRegisterJoinSemesters urj JOIN FETCH urj.student st JOIN FETCH urj.payer pa JOIN FETCH e.schedules sch JOIN FETCH sch.lessonTime JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes  WHERE pa.id = ?1 AND c.id =?2 AND urj.status = 'Completed' ORDER BY e.id")
    List<SemesterClass> findAllSemesterClassByParentAndCourse1(Long student_id, Long course_id);

    @Query("SELECT DISTINCT e FROM SemesterClass e JOIN FETCH e.semester  JOIN FETCH e.course c JOIN FETCH e.userRegisterJoinSemesters urj JOIN FETCH urj.student st JOIN FETCH urj.payer pa JOIN FETCH e.schedules sch JOIN FETCH sch.lessonTime JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes  WHERE pa.id = ?1 AND c.id =?2 AND urj.status = 'Waiting' ORDER BY e.id")
    ...
    @Query("FROM SemesterClass e JOIN FETCH e.course c JOIN FETCH e.semester  JOIN FETCH e.schedules sch JOIN FETCH sch.lessonTime JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes WHERE c.id = ?1 ORDER BY e.id")
    List<SemesterClass> findByCourseId2(Long id);

    @Query("SELECT  DISTINCT e FROM SemesterClass e JOIN FETCH e.course c JOIN FETCH e.semester JOIN FETCH e.schedules sch JOIN FETCH sch.lessonTime JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes WHERE c.id = ?1 ORDER BY e.id")
