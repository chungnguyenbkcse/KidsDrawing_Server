...
public interface SemesterClassRepository extends JpaRepository <SemesterClass, Long>{
    ...
    @Query("SELECT e FROM SemesterClass e JOIN FETCH e.semester  JOIN FETCH e.course WHERE (e.deleted = FALSE OR e.deleted IS NULL) ORDER BY e.id")
    List<SemesterClass> findAll();

    @Query("SELECT  DISTINCT e FROM SemesterClass e JOIN FETCH e.semester  JOIN FETCH e.course c JOIN FETCH e.schedules sch JOIN FETCH sch.lessonTime JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes WHERE (e.deleted = FALSE OR e.deleted IS NULL) ORDER BY e.id")
    ...
    @Query("SELECT DISTINCT e FROM SemesterClass e JOIN FETCH e.semester  JOIN FETCH e.course c JOIN FETCH e.userRegisterJoinSemesters urj JOIN FETCH urj.student st JOIN FETCH e.schedules sch JOIN FETCH sch.lessonTime JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes  WHERE st.id = ?1 AND c.id =?2 AND urj.status = 'Waiting' AND (e.deleted = FALSE OR e.deleted IS NULL) ORDER BY e.id")
    List<SemesterClass> findAllSemesterClassByStudentAndCourseWaiting(Long student_id, Long course_id);

    @Query("SELECT DISTINCT e FROM SemesterClass e JOIN FETCH e.semester  JOIN FETCH e.course c JOIN FETCH e.userRegisterJoinSemesters urj JOIN FETCH urj.student st JOIN FETCH e.schedules sch JOIN FETCH sch.lessonTime JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes  WHERE st.id = ?1 AND c.id =?2 AND urj.status = 'Completed' AND (e.deleted = FALSE OR e.deleted IS NULL) ORDER BY e.id")
    ...
    @Query(
		value = "SELECT e FROM SemesterClass e JOIN FETCH e.semester  JOIN FETCH e.course WHERE (e.deleted = FALSE OR e.deleted IS NULL) ORDER BY e.id",
    ...
    @Query("FROM SemesterClass e JOIN FETCH e.semester  se WHERE se.id = ?1 AND (e.deleted = FALSE OR e.deleted IS NULL) ORDER BY e.id")
    List<SemesterClass> findBySemesterId1(Long id);

    @Query("FROM SemesterClass e JOIN FETCH e.semester se JOIN FETCH e.course c WHERE se.id = ?1 AND (e.deleted = FALSE OR e.deleted IS NULL) ORDER BY e.id")
    List<SemesterClass> findBySemesterId2(Long id);

    @Query("FROM SemesterClass e JOIN FETCH e.course co WHERE co.id = ?1 AND (e.deleted = FALSE OR e.deleted IS NULL) ORDER BY e.id")
    List<SemesterClass> findByCourseId1(Long id);

    @Query("FROM SemesterClass e JOIN FETCH e.course c JOIN FETCH e.semester  JOIN FETCH e.schedules sch JOIN FETCH sch.lessonTime JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes WHERE c.id = ?1 AND (e.deleted = FALSE OR e.deleted IS NULL) ORDER BY e.id")
    List<SemesterClass> findByCourseId2(Long id);

    @Query("SELECT  DISTINCT e FROM SemesterClass e JOIN FETCH e.course c JOIN FETCH e.semester JOIN FETCH e.schedules sch JOIN FETCH sch.lessonTime JOIN FETCH c.artAges JOIN FETCH c.artLevels JOIN FETCH c.artTypes WHERE c.id = ?1 AND (e.deleted = FALSE OR e.deleted IS NULL) ORDER BY e.id")
