...
public interface TutorialTemplatePageRepository extends JpaRepository <TutorialTemplatePage, Long>{
    ...
    @Query("SELECT DISTINCT e FROM TutorialTemplatePage e JOIN FETCH e.sectionTemplate tp JOIN FETCH tp.course co WHERE co.id = ?1 AND (co.deleted = FALSE OR co.deleted IS NULL) AND tp.number = ?2 ORDER BY e.id")
