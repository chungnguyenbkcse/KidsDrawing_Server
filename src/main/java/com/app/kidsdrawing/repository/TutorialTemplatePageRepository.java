...
public interface TutorialTemplatePageRepository extends JpaRepository <TutorialTemplatePage, Long>{
    ...
    @Query("FROM TutorialTemplatePage e JOIN FETCH e.tutorialTemplate WHERE e.id = ?1")
    Optional<TutorialTemplatePage> findById2(Long id);

    @Query("SELECT DISTINCT e FROM TutorialTemplatePage e JOIN FETCH e.tutorialTemplate tp WHERE tp.id = ?1 ORDER BY e.id")
    List<TutorialTemplatePage> findByTutorialTemplateId(Long id);

    @Query("SELECT DISTINCT e FROM TutorialTemplatePage e JOIN FETCH e.tutorialTemplate tp JOIN FETCH tp.sectionTemplate st WHERE st.id = ?1 ORDER BY e.id")
