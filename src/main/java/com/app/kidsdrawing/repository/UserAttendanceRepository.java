...
public interface UserAttendanceRepository extends JpaRepository <UserAttendance, Long>{
    ...
    @Query("FROM UserAttendance e JOIN FETCH e.student  JOIN FETCH e.section WHERE e.id = ?1")
    ...
    @Query("SELECT DISTINCT e FROM UserAttendance e JOIN FETCH e.section se JOIN FETCH e.student  WHERE se.id = ?1 ORDER BY e.id")
    List<UserAttendance> findBySectionId2(Long id);
    
    boolean existsById(Long id);

    @Query("SELECT DISTINCT e FROM UserAttendance e JOIN FETCH e.student st WHERE st.id = ?1 ORDER BY e.id")
    List<UserAttendance> findByStudentId1(Long id);

    @Query("SELECT DISTINCT e  FROM UserAttendance e JOIN FETCH e.student st JOIN FETCH e.section WHERE st.id = ?1 ORDER BY e.id")
    ...
    @Query("SELECT DISTINCT e FROM UserAttendance e JOIN FETCH e.student st JOIN FETCH st.parent pa JOIN FETCH e.section se WHERE se.id = ?1 AND pa.id = ?2 ORDER BY e.id")
    ...
    @Query("SELECT DISTINCT e FROM UserAttendance e JOIN FETCH e.student st JOIN FETCH e.section se JOIN FETCH se.classes cl WHERE cl.id = ?1 ORDER BY e.id")
