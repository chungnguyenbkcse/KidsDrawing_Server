...
public interface ExerciseSubmissionRepository extends JpaRepository <ExerciseSubmission, Long>{
    ...
    @Query("SELECT DISTINCT e FROM ExerciseSubmission e JOIN FETCH e.exercise JOIN FETCH e.student st JOIN FETCH st.user ORDER BY e.create_time DESC")
    List<ExerciseSubmission> findAll();

    @Query("SELECT DISTINCT e FROM ExerciseSubmission e JOIN FETCH e.exercise ex  JOIN FETCH e.student st JOIN FETCH st.user JOIN FETCH ex.section se JOIN FETCH se.classes cl WHERE cl.id =?1 AND st.id =?2  ORDER BY e.create_time DESC")
    List<ExerciseSubmission> findAllExerciseSubmissionByClassAndStudent(Long class_id, Long student_id);

    @Query("SELECT DISTINCT e FROM ExerciseSubmission e JOIN FETCH e.exercise ex JOIN FETCH e.student st JOIN FETCH st.user JOIN FETCH ex.section se JOIN FETCH se.classes cl WHERE se.id =?1 AND st.id =?2 ORDER BY e.create_time DESC")
    List<ExerciseSubmission> findAllExerciseSubmissionBySectionAndStudent(Long section_id, Long student_id);

    @Query("SELECT DISTINCT e FROM ExerciseSubmission e JOIN FETCH e.exercise ex JOIN FETCH e.student  st  JOIN FETCH st.user JOIN FETCH st.parent pa JOIN FETCH pa.user JOIN FETCH ex.section se WHERE se.id =?1 AND pa.id =?2  AND (ex.deleted = FALSE OR ex.deleted IS NULL)  ORDER BY e.create_time DESC")
    List<ExerciseSubmission> findAllExerciseSubmissionBySectionAndParent(Long section_id, Long parent_id);

    @Query("SELECT DISTINCT e FROM ExerciseSubmission e JOIN FETCH e.exercise ex JOIN FETCH e.student st JOIN FETCH st.user JOIN FETCH ex.section se WHERE se.id =?1 AND st.id =?2  AND (ex.deleted = FALSE OR ex.deleted IS NULL)  ORDER BY e.create_time DESC")
    List<ExerciseSubmission> findAllExerciseSubmissionBySectionAndStudent1(Long section_id, Long student_id);

    @Query("SELECT DISTINCT e FROM ExerciseSubmission e JOIN FETCH e.exercise ex JOIN FETCH e.student st JOIN FETCH st.user JOIN FETCH ex.section se JOIN FETCH se.classes cl WHERE cl.id =?1  ORDER BY e.create_time DESC")
    ...
    @Query(
		value = "SELECT e FROM ExerciseSubmission e JOIN FETCH e.exercise JOIN FETCH e.student st JOIN FETCH st.user ORDER BY e.create_time DESC",
    ...
    @Query("SELECT DISTINCT e FROM ExerciseSubmission e JOIN FETCH e.exercise ex JOIN FETCH e.student st JOIN FETCH st.user WHERE ex.id = ?1  ORDER BY e.create_time DESC")
