...
public interface ClassesRepository extends JpaRepository <Classes, Long>{
    ...
    @Query("FROM Classes c  JOIN FETCH c.userRegisterTeachSemester WHERE (c.deleted = FALSE OR c.deleted IS NULL)")
    ...
    @Query("SELECT c FROM Classes c JOIN FETCH c.userRegisterTeachSemester WHERE c.id = ?1 AND (c.deleted = FALSE OR c.deleted IS NULL)")
