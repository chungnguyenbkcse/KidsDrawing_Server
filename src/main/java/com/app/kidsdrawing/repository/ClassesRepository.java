...
public interface ClassesRepository extends JpaRepository <Classes, Long>{
    ...
    @Query("SELECT DISTINCT c1 FROM Classes c1  JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH c1.semesterClass sc JOIN FETCH sc.schedules sch JOIN FETCH sch.lessonTime lt JOIN FETCH sc.semester s LEFT JOIN FETCH s.holidays JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH sc.course c JOIN FETCH c.artLevels al JOIN FETCH c.artTypes at JOIN FETCH c.artAges ag WHERE (c1.deleted = FALSE OR c1.deleted IS NULL) AND (c.deleted = FALSE OR c.deleted IS NULL) AND (at.deleted = FALSE OR at.deleted IS NULL) AND (ag.deleted = FALSE OR ag.deleted IS NULL) AND (al.deleted = FALSE OR al.deleted IS NULL) AND (lt.deleted = FALSE OR lt.deleted IS NULL) AND (sc.deleted = FALSE OR sc.deleted IS NULL) AND (s.deleted = FALSE OR s.deleted IS NULL)")
    List<Classes> findAll1();

    @Query("SELECT DISTINCT c1 FROM Classes c1  JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH c1.semesterClass sc JOIN FETCH sc.schedules sch JOIN FETCH sch.lessonTime lt JOIN FETCH sc.semester s LEFT JOIN FETCH s.holidays WHERE (c1.deleted = FALSE OR c1.deleted IS NULL) AND (lt.deleted = FALSE OR lt.deleted IS NULL) AND (sc.deleted = FALSE OR sc.deleted IS NULL) AND (s.deleted = FALSE OR s.deleted IS NULL)")
    List<Classes> findAll2();

    @Query("SELECT DISTINCT c1 FROM Classes c1 JOIN FETCH c1.classHasRegisterJoinSemesterClasses crj JOIN FETCH crj.student st JOIN FETCH st.user stu  JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH c1.semesterClass sc JOIN FETCH sc.schedules sch JOIN FETCH sch.lessonTime lt JOIN FETCH sc.semester s LEFT JOIN FETCH s.holidays JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH sc.course c JOIN FETCH c.artLevels al JOIN FETCH c.artTypes at JOIN FETCH c.artAges ag WHERE te.id = ?1 AND (c.deleted = FALSE OR c.deleted IS NULL) AND (c1.deleted = FALSE OR c1.deleted IS NULL) AND (at.deleted = FALSE OR at.deleted IS NULL) AND (ag.deleted = FALSE OR ag.deleted IS NULL) AND (al.deleted = FALSE OR al.deleted IS NULL) AND (lt.deleted = FALSE OR lt.deleted IS NULL) AND (sc.deleted = FALSE OR sc.deleted IS NULL) AND (s.deleted = FALSE OR s.deleted IS NULL) AND (stu.deleted = FALSE OR stu.deleted IS NULL)")
    ...
    @Query("SELECT DISTINCT c1 FROM Classes c1 JOIN FETCH c1.classHasRegisterJoinSemesterClasses crj JOIN FETCH crj.student st  JOIN FETCH st.user stu JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH c1.semesterClass sc JOIN FETCH sc.schedules sch JOIN FETCH sch.lessonTime lt JOIN FETCH sc.semester s LEFT JOIN FETCH s.holidays JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH sc.course c JOIN FETCH c.artLevels al JOIN FETCH c.artTypes at JOIN FETCH c.artAges  ag WHERE st.id = ?1 AND (c1.deleted = FALSE OR c1.deleted IS NULL) AND (c.deleted = FALSE OR c.deleted IS NULL) AND (at.deleted = FALSE OR at.deleted IS NULL)  AND (ag.deleted = FALSE OR ag.deleted IS NULL) AND (al.deleted = FALSE OR al.deleted IS NULL) AND (lt.deleted = FALSE OR lt.deleted IS NULL) AND (sc.deleted = FALSE OR sc.deleted IS NULL) AND (s.deleted = FALSE OR s.deleted IS NULL) AND (stu.deleted = FALSE OR stu.deleted IS NULL)")
    ...
    @Query("SELECT DISTINCT c1 FROM Classes c1 JOIN FETCH c1.classHasRegisterJoinSemesterClasses crj JOIN FETCH crj.student st JOIN FETCH st.user stu JOIN FETCH st.parent pa  JOIN FETCH pa.user JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH pa.user JOIN FETCH c1.semesterClass sc JOIN FETCH sc.schedules sch JOIN FETCH sch.lessonTime lt JOIN FETCH sc.semester s LEFT JOIN FETCH s.holidays JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH sc.course c JOIN FETCH c.artLevels al JOIN FETCH c.artTypes at JOIN FETCH c.artAges ag WHERE pa.id = ?1 AND (c1.deleted = FALSE OR c1.deleted IS NULL) AND (c.deleted = FALSE OR c.deleted IS NULL) AND (at.deleted = FALSE OR at.deleted IS NULL) AND (ag.deleted = FALSE OR ag.deleted IS NULL) AND (al.deleted = FALSE OR al.deleted IS NULL) AND (lt.deleted = FALSE OR lt.deleted IS NULL) AND (sc.deleted = FALSE OR sc.deleted IS NULL) AND (s.deleted = FALSE OR s.deleted IS NULL) AND (stu.deleted = FALSE OR stu.deleted IS NULL)")
    ...
    @Query("SELECT DISTINCT c1 FROM Classes c1 JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH c1.semesterClass sc JOIN FETCH sc.course c JOIN FETCH sc.schedules JOIN FETCH sc.semester s LEFT JOIN FETCH s.holidays WHERE c1.id = ?1 AND (c1.deleted = FALSE OR c1.deleted IS NULL) AND (sc.deleted = FALSE OR sc.deleted IS NULL) AND (c.deleted = FALSE OR c.deleted IS NULL) AND (s.deleted = FALSE OR s.deleted IS NULL)")
    Optional<Classes> findById3(Long id);

    @Query("SELECT DISTINCT c1 FROM Classes c1 JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH c1.semesterClass sc JOIN FETCH sc.course c JOIN FETCH sc.schedules sch JOIN FETCH sch.lessonTime lt JOIN FETCH sc.semester s LEFT JOIN FETCH s.holidays WHERE c1.id = ?1 AND (c.deleted = FALSE OR c.deleted IS NULL) AND (c1.deleted = FALSE OR c1.deleted IS NULL) AND (lt.deleted = FALSE OR lt.deleted IS NULL) AND (sc.deleted = FALSE OR sc.deleted IS NULL) AND (s.deleted = FALSE OR s.deleted IS NULL)")
    Optional<Classes> findById4(Long id);

    @Query("SELECT c1 FROM Classes c1 JOIN FETCH c1.classHasRegisterJoinSemesterClasses  chr JOIN FETCH chr.student st  JOIN FETCH c1.teacher te JOIN FETCH te.user JOIN FETCH c1.semesterClass sc JOIN FETCH sc.course c JOIN FETCH sc.schedules sch JOIN FETCH sch.lessonTime lt JOIN FETCH sc.semester s LEFT JOIN FETCH s.holidays JOIN FETCH c.artLevels al JOIN FETCH c.artTypes at JOIN FETCH c.artAges ag WHERE c1.id = ?1 AND (c1.deleted = FALSE OR c1.deleted IS NULL) AND (c.deleted = FALSE OR c.deleted IS NULL) AND (at.deleted = FALSE OR at.deleted IS NULL) AND (ag.deleted = FALSE OR ag.deleted IS NULL) AND (al.deleted = FALSE OR al.deleted IS NULL) AND (lt.deleted = FALSE OR lt.deleted IS NULL) AND (sc.deleted = FALSE OR sc.deleted IS NULL) AND (s.deleted = FALSE OR s.deleted IS NULL)")
